...
let nooseWord = wordsToPickFrom[randomInt(wordsToPickFrom.length)]

function createArray(wrd){
    let letterSpaces = []
...
function hangman(){
    ...
    console.log(" Welcome To Hangman\n","Try to guess the word, but dont let John Doe die")
    let letterSpaces = createArray(nooseWord)
    let counter = 0
    
    console.log(letterSpaces.join(" "))

        while(!(letterSpaces.join("") == nooseWord)){
        let guess = readline.question("guess a letter: ")
        
        
                for (i = 0; i <nooseWord.length; i++){
                    if( nooseWord[i] == guess ){

                        letterSpaces[i] = guess
                    }
                }
        ...
        console.log("The word was", letterSpaces.join(""), "and it only took you " + counter + " tries to guess a ", nooseWord.length, " letter word. GG\m", "Your accuracy is ",Math.round((nooseWord.length/counter)*100) + "%")
        return letterSpaces
